...
get_estimates_ONEparam.AvgMed<-function(dat,timeseries,stations,colname.param){
...
data.param_month<-as.data.frame(matrix(NA,ncol=(4+length(Nobs_stations)),nrow=(length(months)*length(timeseries)),
                                     dimnames=list(NULL,c("year","month",Nobs_stations,"monthly_AVG","monthly_MED"))))
nrow.df<-0
for(i in 1:length(timeseries)){
  for(u in 1:length(months)){
    ...
    monthly.median<-vector(mode="numeric",length=length(stations))
    for(p in 1:length(stations)){
     ...
     data.param_month[nrow.df, Nobs_stations[p]]<-nrow(mydat2[which(is.na(mydat2[,colname.param])==FALSE),])
      ...
      monthly.median[p]<-median(mydat2[,colname.param],na.rm=TRUE)}#monthly MEDIAN per station
    data.param_month[nrow.df,"monthly_AVG"]<-mean(monthly.mean,na.rm=TRUE)#monthly AVG across stations
    data.param_month[nrow.df,"monthly_MED"]<-mean(monthly.median,na.rm=TRUE)#monthly MEDIAN across stations
    }}

#2.seasonal AVG + MEDIAN of SRB
#-->seasonal AVG + MEDIAN calculated based on monthly AVG
...
data.param_season<-as.data.frame(matrix(NA,ncol=(4+length(Nobs_stations)),nrow=(length(timeseries)*4),
                                      dimnames=list(NULL,c("year","season",Nobs_stations,"seasonal_AVG","seasonal_MED"))))
...
for(i in 1:length(timeseries)){
  ...
  for(u in 1:length(season)){
    ...
    data.param_season[nrow.df,"seasonal_MED"]<-mean(mydat$monthly_MED,na.rm=TRUE)#seasonal mean median calculated based on monthly MEDIANS
    }}
...
data.param_annual<-as.data.frame(matrix(NA,ncol=(3+length(Nobs.sum_stations)),nrow=length(timeseries),
                                        dimnames=list(NULL,c("year",Nobs.sum_stations,"annual_AVG","annual_MED"))))
nrow.df<-0
for(i in 1:length(timeseries)){
  nrow.df<-nrow.df+1
  ...
  data.param_annual[nrow.df,"annual_MED"]<-mean(mydat$monthly_MED,na.rm=TRUE)#annual mean MEDIAN
